 ...
 * 기존 홈페이지 구성은 유지하고, 기존 DB/메일 서버 API로 실제 전송되도록 수정
 */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || data.ok === false || data.success === false) {
        throw new Error(data.error || "상담 신청 처리에 실패했습니다.");
      }
      ...
      <div className="container relative z-10" ref={ref}>
        ...
        >
          ...
          <p className="max-w-2xl mx-auto text-gray-400 leading-relaxed">
            히든 피씨카페 창업에 관심이 있으시다면 아래 양식을 작성해 주세요.
        ...
        <div className="grid lg:grid-cols-5 gap-10 lg:gap-12">
              ...
              <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-6 sm:p-8 space-y-5">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="consulting">상담 우선</option>
                  </select>
                </div>
          ...
          >
            ...
            <motion.a
              href="tel:1866-1508"
              className="block glass-card rounded-xl p-6 bg-gradient-to-r from-purple-600/10 to-pink-600/10 border-purple-500/30 hover:border-purple-400/50 transition-all duration-300 text-center"
              initial={{ opacity: 0, y: 20 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: 0.9 }}
            >
              <Phone className="w-6 h-6 text-purple-400 mx-auto mb-3" />
              <div className="text-lg font-bold text-white mb-1">지금 바로 전화 상담</div>
              <div className="text-sm text-gray-400">1866-1508</div>
            </motion.a>
          </motion.div>
        </div>
      </div>
